...
class Node:
    ...
    def __init__(self, name, tr, proc, resp):
        self.name = name
        self.response = resp
        self.transitions = tr
    ...
    @classmethod
    def parse_node(cls, node_name, node, imports, source):
        transitions = {}
        processing = {}
        response = ""
        for key, value in zip(node.keys, node.values):
            if key.id == 'TRANSITIONS':
                ...
                for tr_k, tr_v in zip(value.keys, value.values):
                    ...
                    transitions[tr_k.value] = tr_description
        ...
        return cls(node_name, transitions, processing, response)
                ...
                for key, value in zip(child.keys, child.values):
                    ...
                    if flow_name == "GLOBAL":
                        self.global_flow['GLOBAL'] = Node.parse_node(
                            flow_name, value, self.imports, self.source)
                    else:
                        local_flow = {}
                        for node_key, node_val in zip(value.keys, value.values):
                            node_name = self.get_name(node_key)
                            local_flow[node_name] = Node.parse_node(
                                node_name, node_val, self.imports, self.source)
...
def graph2drawio(graph):
    ...
    """
    head = """<mxfile host="65bd71144e" scale="1" border="0">
    ...
    for local_flow, flow_data in graph.items():
        for node, data in flow_data.items():
            node_text = f"""
                <UserObject label="{node}" flow="{local_flow}" response="{data['response']}" processing="{str(data['processing'])}" id="{data['id']}">
            ...
            for target, edge_data in data['edges'].items():
                edge_text = f"""
                    <mxCell id="{edge_data['id']}" flow="{local_flow}" value="{edge_data['title']}" style="edgeStyle=orthogonalEdgeStyle;rounded=0;orthogonalLoop=1;jettySize=auto;html=1;noEdgeStyle=1;orthogonal=1;" parent="2" source="{data["id"]}" target="{flow_data[target]['id']}" edge="1">
