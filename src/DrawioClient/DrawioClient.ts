 ...
 */
export class DrawioClient<TCustomAction extends {} = never, TCustomEvent extends {} = never> {
  ...
  constructor(
    ...
    private readonly selected_predictor: VsCodeSetting<string>,
    private readonly sfc_url: VsCodeSetting<string>,
    private readonly midas_url: VsCodeSetting<string>
  ) {
    this.openedForms = {};
    this.openedDFFs = {};
    this.dispose.track(
  ...
  protected sendCustomActionExpectResponse(action: TCustomAction): Promise<TCustomEvent> {
  ...
  private sendActionWaitForResponse(action: TCustomAction): Promise<TCustomEvent>;
  private sendActionWaitForResponse(
    ...
    return new Promise((resolve, reject) => {
      ...
      this.messageStream.sendMessage(JSON.stringify(Object.assign(action, { actionId })));
    });
  }
  ...
  protected async handleEvent(evt: { event: string }): Promise<void> {
    ...
    if ("message" in drawioEvt) {
      const actionId = (drawioEvt.message as any).actionId as string | undefined;
      if (actionId) {
      ...
      req.on("error", (error) => {
        ...
        this.vwP.webview.postMessage({
          connectionError: "showDFFConnError",
        });
        console.log(error);
      });
      ...
      var defaultSuggs: any[] = [
        { sug: "Custom condition", conf: 1 },
        { sug: "exact_match", conf: 1 },
        { sug: "regex_match", conf: 1 },
      ];
      ...
      if (delta > 500) {
        ...
        if (predictor === "sfc") {
          ...
          rp(getReqOpts)
            ...
            .catch((err: any) => {
              // POST failed, render default suggestions
              console.error(err);
          ...
          const getReqOpts = (classes: string[]) => ({
            method: "POST",
            uri: this.midas_url.get(),
            body: { last_midas_labels: classes, return_probas: 1 },
            json: true, // Automatically stringifies the body to JSON
          });
          rp(getReqOpts(midasClasses))
            .then(async (userPreds: { [k: string]: number }[]) => {
              const nodes_suggestions: any = {};
              const cells_i: any[] = [];
              for (let i = 0; i < userPreds.length; i++) {
                const cel = drawioEvt.cells[i];
                const userPred = userPreds[i];
                const suggestions = [
                  ...Object.entries(userPred)
                    .map(([sug, conf]) => ({ sug, conf }))
                    .sort(({ conf: a }, { conf: b }) => (a < b ? -1 : 1)),
                  ...defaultSuggs,
                ];
                cells_i.push({
                  x: cel.x,
                ...
                const userClasses = Object.keys(userPred);
                const botPreds: { [k: string]: number }[] = await rp(getReqOpts(userClasses));
                for (let j = 0; j < userClasses.length; j++) {
                  const userClass = userClasses[j];
                  const botPred = botPreds[j];
                  const nodeSuggestion = Object.entries(botPred).map(([sug, conf]) => ({
                    sug,
                    conf,
                  }));
                  nodes_suggestions[userClass] = nodeSuggestion;
                }
              }
      ...
      if (!this.openedForms.hasOwnProperty(cid)) {
            ...
            switch (message.command) {
              case "saveAsPng":
                ...
                if (drawioEvt.suggs) {
                  ...
                  let midas = form_data.midas.split(" ")[0];
                  if (midas != "" && !midas.endsWith('"')) midas += '"';
                  this.addSuggNode({
                  ...
                  (webviewPanel as any)._drawiovw.postMessage({
                    oleg: "Privet",
                    cell_id: cid,
        ...
        webviewPanel.onDidDispose(
          () => {
            delete this.openedForms[(webviewPanel as any)._cid];
          },
          null,
          this.context.subscriptions
        );
        if (!drawioEvt.suggs) {
          const idx = node_info.nodeNames.indexOf(
            node_info.title.replace(/^f?['"]/, "").replace(/['"]$/, "")
          );
  ...
  private setHtmlContent(
    ...
    var midas_or_sf_suggs: any = [];
    if (node_info["suggs"]) {
      var suggs: { sug: string; conf: number }[] = JSON.parse(node_info["suggs"]);
      suggs
        .sort(({ conf: a }, { conf: b }) => (a > b ? -1 : 1))
        .forEach((sug: any) => {
          var s = sug["sug"];
          var c = Number(sug["conf"]).toFixed(2);
          midas_or_sf_suggs.push(`${s} ${c}`);
        });
    }
    let speech_functions: string[] = [];
    let midas_classes: string[] = [];
    let predictorUsed: string | undefined;
    if (midas_or_sf_suggs.length == 0) {
      speech_functions = base_speech_functions;
      midas_classes = base_midas_classes;
      midas_classes = base_midas_classes;
    } else if (midas_or_sf_suggs[0].match(/^[A-Z].*/)) {
      predictorUsed = "sfc";
      speech_functions = midas_or_sf_suggs;
      midas_classes = base_midas_classes;
    } else {
      predictorUsed = "midas";
      midas_classes = midas_or_sf_suggs;
      speech_functions = base_speech_functions;
    }
    speech_functions = ["----", ...speech_functions];
    midas_classes = ["----", ...midas_classes];
    ...
    var node_info_midas = node_info["cell_content"]["midas"];
    if (!node_info_midas) node_info_midas = "";
    let htmlContent2 = `
		<head>
		  <meta charset="UTF-8">
      ...
      <script >window.nodeNames = ${JSON.stringify(node_info.nodeNames || [])};</script>
		...
		<div class="formbuilder-text form-group field-text-1627806340486">
			...
			<input type="text" placeholder="Node Title" class="form-control" 
			       name="node_title" access="false" value="${quoteattr(
               title.replace(/^f?['"]/, "").replace(/['"]$/, "")
             )}" 
				   id="b">
    ...
    speech_functions.forEach((element: any, idx: number) => {
      ...
      if (
        element.split(" ")[0] === sfc_name ||
        (predictorUsed === "sfc" && node_info["suggs"] && idx === 1) ||
        (sfc_name === "" && idx === 0)
      )
    ...
    htmlContent2 += '</select><h3>MIDAS</h3><select name="midas" id="midas-selector">';
    midas_classes.forEach((element: any, idx: number) => {
      let midas_name = node_info_midas.replace(/^f?['"]/, "").replace(/['"]$/, "");
      htmlContent2 += `<option value="${element}"`;
      if (
        element.split(" ")[0] === midas_name ||
        (predictorUsed === "midas" && node_info["suggs"] && idx === 1) ||
        (midas_name === "" && idx === 0)
      )
        htmlContent2 += ` selected`;
    ...
    htmlContent2 += `</select>
      ...
      <h3 id="example-title">SFC Examples:</h3>
    ...
    //   var children_el =
    //     `<li>		` +
    //     `<input type="button" class="connected_node_node" id="${children_cell_id}" value="${quoteattr(
    //       children_title
    //     )}">` +
    ...
    if (dataUrl.length > 0) {
      ...
      this.writeFile(newFilePath, dataUrl, () => {
        vscode.window.showInformationMessage(
          `The file ${newFilePath} has been created in the root of the workspace.`
        );
      });
    }
  }
    ...
    if (currFile.endsWith(".py")) {
      this.convertPyData()
        .then((result) => {
          ...
          this.vwP.webview.postMessage({
            graphOperations: "rearrangeGraph",
          });
        })
        .catch((error) => {
          console.log(error);
  ...
  public async export(extension: string): Promise<Buffer> {
    ...
    } else if (extension.endsWith(".dff")) {
